...
public interface AppelDeJourRepository extends JpaRepository<AppelDeJour, Date> {
	...
	@Query(value = "SELECT p.nom,p.prenom,p.nom_AR,p.prenom_AR,p.poste_Occupe,p.matricule,p.id_personnel,a.datedujour from appel_de_jour_personnels c join personnel p on c.personnels_id_personnel=p.id_personnel join appel_de_jour a on c.appels_datedujour=a.datedujour where a.etat='Sansjusitf';", nativeQuery = true)
